	...
	{foreach $node.object.allowed_assign_state_list as $group}
		{if $display_groups|contains($group.group.identifier)}
			...
			<li id="node-tab-{$group_identifier}" class="{if $i|eq($display_groups|count)}last{else}middle{/if}{if $node_tab_index|eq($group_identifier)} selected{/if}">
		        {if $tabs_disabled}
		            <span class="disabled" title="{'Tab is disabled, enable with toggler to the left of these tabs.'|i18n( 'design/admin/node/view/full' )}">{$group_name|i18n( 'extension/oweditorial' )}</span>
		        {else}
		            <a href={concat( $node_url_alias, '/(tab)/', $group_identifier )|ezurl} title="{'Show editorial overview.'|i18n( 'extension/oweditorial' )}">{$group_name|i18n( 'extension/oweditorial' )}</a>
		        {/if}
		    </li>
